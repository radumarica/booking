...
const ImageCheckbox: React.FC<TypeImageCheckboxProps> = ({
  ...
  if (validity === false && isTouched)
    wrapperClasses.push(checkboxClasses['invalid']);
  wrapperClasses.push(checkboxClasses['image-checkboxes']);
  if (!label)
    wrapperClasses.push(checkboxClasses['checkbox--no-label']);
            ...
            options.map((option: TypeOption, index) => (
              ...
              >
                ...
                >
                  <h5
                    className={
                      checkboxClasses['image-checkbox__label__title']
                    }
                  >
                    {option.label}
                  </h5>
                  {option.imgUrl && (
                    <div
                      className={
                        checkboxClasses[
                          'image-checkbox__label__img-wrapper'
                        ]
                      }
                    >
                      <img
                  ...
                  {option.meta && (
                    <p
                      className={
                        checkboxClasses['image-checkbox__label__meta']
                      }
                    >
                      {option.meta}
                    </p>
 ...
 */
function areEqual(
  prevProps: TypeImageCheckboxProps,
  nextProps: TypeImageCheckboxProps
) {
  const compareKeys = [
    'options',
    'value',
  ...
  const areEqual = compareProps<TypeImageCheckboxProps>(
    prevProps,
    nextProps,
    compareKeys
  );
